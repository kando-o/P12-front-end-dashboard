...
export const DataProvider = ({ children }) => {
	...
	const [errors, setErrors] = useState([]); // Nouvel état pour stocker toutes les erreurs
				...
				if (id==="mocked") {
					...
					setTimeout(async () => {
						try {
							...
							console.error('Fetch error:', error.message, `error : ${error.message}`);
							
							// Amélioration du message d'erreur pour les erreurs réseau
							let errorMessage = error.message;
							if (error.message.includes("Failed to fetch")) {
								errorMessage = "Erreur de connexion : Impossible de se connecter au serveur. Veuillez vérifier votre connexion réseau ou que le serveur est démarré.";
							}
							
							// Ajouter l'erreur à la liste au lieu d'écraser
							setErrors(prev => [...prev, { type: 'user', message: errorMessage }]);
							setErrorData(errorMessage); // On passe le message d'erreur à l'état
							return `Aucun ID disponible server indisponible`
						} finally {
					...
					setTimeout(async () => {
						try {
							...
							let errorMessage = error.message;
							if (error.message.includes("Failed to fetch")) {
								errorMessage = "Erreur de connexion : Impossible de récupérer les données d'activité. Veuillez vérifier votre connexion réseau.";
							}
							
							// Ajouter l'erreur à la liste au lieu d'écraser
							setErrors(prev => [...prev, { type: 'activity', message: errorMessage }]);
							setErrorData(errorMessage);  // Met l'erreur dans l'état pour affichage
						} finally {
					...
					setTimeout(async () => {
						try {
							...
							let errorMessage = error.message;
							if (error.message.includes("Failed to fetch")) {
								errorMessage = "Erreur de connexion : Impossible de récupérer les sessions moyennes. Veuillez vérifier votre connexion réseau.";
							}
							
							// Ajouter l'erreur à la liste au lieu d'écraser
							setErrors(prev => [...prev, { type: 'sessions', message: errorMessage }]);
					...
					setTimeout(async () => {
						try {
							...
							let errorMessage = error.message;
							if (error.message.includes("Failed to fetch")) {
								errorMessage = "Erreur de connexion : Impossible de récupérer les données de performance. Veuillez vérifier votre connexion réseau.";
							}
							
							// Ajouter l'erreur à la liste au lieu d'écraser
							setErrors(prev => [...prev, { type: 'performance', message: errorMessage }]);
							setErrorData(errorMessage);
						} finally {
	...
	return <DataContext.Provider value={{loadingData, loadingActivity, loadingPerformance, loadingAvgSessions, errorData, errors, userData, userActivityData, userAverageSessions, userPerformance, id, setId }}>
		{children}
